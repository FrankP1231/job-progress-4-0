...
export const getInProgressPhases = async (): Promise<{ job: Job, phase: Phase }[]> => {
  ...
  for (const item of phaseData || []) {
    const phase: Phase = {
      ...
      weldingMaterials: item.welding_materials,
      sewingMaterials: item.sewing_materials,
      weldingLabor: item.welding_labor,
      sewingLabor: item.sewing_labor,
      installationMaterials: item.installation_materials,
      powderCoat: item.powder_coat,
